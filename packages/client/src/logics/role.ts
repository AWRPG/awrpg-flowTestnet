...
/**
 * note1: idealy, should've load poolTypes from RoleSpecs.maxPools, but use hook cannot put in loop; thus, for simplicity, call each poolInfo separately;
 ...
 * note3: two tables are subject to change/updated: 1) RoleSpecs table (when player updates his role type), 2) Balance tables when pool amount changes
 */
